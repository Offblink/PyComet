...
class WorkingDirectoryDialog(QDialog):
    ...
    def __init__(self, parent=None, current_dir=""):
        ...
        # 说明标签
        info_label = QLabel("请选择代码运行时的工作目录：")
        layout.addWidget(info_label)
        
        # 目录选择布局
        dir_layout = QHBoxLayout()
        self.dir_input = QLineEdit()
        self.dir_input.setPlaceholderText("选择工作目录...")
        ...
        # 提示标签
        tip_label = QLabel("提示：代码运行时将在此目录下创建临时文件并执行")
    ...
    def browse_directory(self):
        ...
        dir_path = QFileDialog.getExistingDirectory(
            self, "选择工作目录", "", QFileDialog.ShowDirsOnly
        )
        if dir_path:
...
class CometIDE(QMainWindow):
    ...
    def init_ui(self):
        ...
        self.console_input.setPlaceholderText("在此输入内容后按回车发送...")
        self.console_input.setEnabled(False)
        self.console_input.returnPressed.connect(self.on_console_input_submit)
        ...
        # 工作目录显示标签
        self.working_dir_label = QLabel()
        self.update_working_dir_display()
        self.status_bar.addWidget(self.working_dir_label)
            ...
            try:
                with open(config_file, 'r', encoding='utf-8') as f:
                    ...
                    if 'working_directory' in config:
                        result['working_directory'] = config.get('working_directory', '')
    ...
    def save_config(self, api_key=None, working_directory=None):
        """保存配置到文件（包含API密钥和工作目录）"""
        config_file = "comet_config.json"
    ...
    def set_working_directory(self):
        """设置工作目录（菜单调用）"""
        dialog = WorkingDirectoryDialog(self, self.working_directory)
        if dialog.exec_() == QDialog.Accepted:
            new_dir = dialog.get_directory()
            if new_dir:
                self.working_directory = new_dir
                if self.save_config(working_directory=new_dir):
                    self.console_update_signal.emit(f"工作目录已设置为: {new_dir}", "normal")
                else:
                    QMessageBox.warning(self, "保存失败", "保存工作目录配置失败")
            else:
                self.working_directory = ""
                self.save_config(working_directory="")
                self.console_update_signal.emit("工作目录已清除，将使用默认临时目录", "normal")
            # 更新状态栏显示
            self.update_working_dir_display()
    
    def update_working_dir_display(self):
        """更新状态栏中工作目录的显示"""
        if self.working_directory and os.path.isdir(self.working_directory):
            # 缩短路径显示，只显示最后两级目录
            parts = self.working_directory.replace('\\', '/').split('/')
            if len(parts) > 2:
                display_path = ".../" + "/".join(parts[-2:])
            else:
                display_path = self.working_directory
            self.working_dir_label.setText(f"📂 工作目录: {display_path}")
        else:
            self.working_dir_label.setText("📂 工作目录: 未设置（使用系统临时目录）")
    ...
    def create_shared_actions(self):
        ...
        # 工作目录相关的共享action
        self.set_working_dir_action = QAction("设置工作目录", self)
        self.set_working_dir_action.triggered.connect(self.set_working_directory)
    ...
    def create_menu(self):
        ...
        settings_menu.addAction(self.set_working_dir_action)
    ...
    def autosave(self):
        """自动保存当前内容 - 将文件保存到工作目录或临时目录"""
        try:
            content = self.editor_panel.toPlainText()
            
            # 确定自动保存文件的位置
            if self.working_directory and os.path.isdir(self.working_directory):
                autosave_path = os.path.join(self.working_directory, ".comet_ide_autosave.py")
            else:
                autosave_path = os.path.join(tempfile.gettempdir(), ".comet_ide_autosave.py")
    ...
    def execute_with_input(self, code):
        """执行需要用户交互输入的代码 - 通过内置控制台交互"""
        try:
            temp_file_path = self.get_autosave_path()
            with open(temp_file_path, "w", encoding="utf-8") as f:
                f.write(code)

            is_frozen = getattr(sys, "frozen", False)
            python_cmd = sys.executable if not is_frozen else "python"

            # 在Windows打包环境中，使用CREATE_NO_WINDOW标志防止子进程打开新窗口
            creationflags = 0
            ...
            def read_stdout():
                for line in self.input_process.stdout:
                    if line:
                        self.console_update_signal.emit(line.rstrip(), "normal")

            def read_stderr():
                for line in self.input_process.stderr:
                    if line:
            ...
            def wait_for_process():
                ...
                returncode = self.input_process.returncode
                QMetaObject.invokeMethod(self.console_input, "setEnabled", Qt.QueuedConnection, Q_ARG(bool, False))
                self.is_interactive_mode = False
                if returncode == 0:
                ...
                self.is_running = False
                QMetaObject.invokeMethod(self.stop_button, "setEnabled", Qt.QueuedConnection, Q_ARG(bool, False))
                try:
    ...
    def on_console_input_submit(self):
        """处理控制台输入框的提交"""
        if not self.is_interactive_mode or not self.input_process:
            return
        user_input = self.console_input.text()
        if self.input_process.poll() is None:
            try:
                self.input_process.stdin.write(user_input + "\n")
                self.input_process.stdin.flush()
                self.console_update_signal.emit(f">>> {user_input}", "normal")
            except Exception as e:
                self.console_update_signal.emit(f"输入错误: {e}", "failure")
        self.console_input.clear()
    ...
    def execute_non_interactive(self, code):
        ...
        # 在指定目录创建临时文件
        fd, temp_file = tempfile.mkstemp(suffix='.py', prefix='comet_', dir=temp_dir, text=True)
        try:
...
4. 仅提供代码与注释，不需要多余的文字说明"""
    ...
    def get_autosave_path(self):
        """获取自动保存文件的路径"""
        if self.working_directory and os.path.isdir(self.working_directory):
            return os.path.join(self.working_directory, ".comet_ide_autosave.py")
        else:
            return os.path.join(tempfile.gettempdir(), ".comet_ide_autosave.py")
    ...
    def save_code(self):
        ...
        try:
            ...
            self.console_update_signal.emit(f"代码已保存到自动保存文件: {autosave_path}", "normal")
        except Exception as e:
    ...
    def open_file(self):
        """打开文件 - 增加初始化自动补全词汇，设置工作目录为文件所在目录"""
        file_path, _ = QFileDialog.getOpenFileName(
        ...
        if file_path:
            try:
                ...
                self.save_config(working_directory=file_dir)
                self.update_working_dir_display()
                
                self.console_update_signal.emit(f"已打开文件: {file_path}", "normal")
                self.console_update_signal.emit(f"工作目录已设置为: {file_dir}", "normal")
            except Exception as e:
    ...
    def package_app(self):
        ...
        try:
            ...
            self.console_update_signal.emit(f"✓ 已更新自动保存文件: {autosave_path}", "normal")
        except Exception as e:
    ...
    def execute_packaging_thread(self, app_name, icon_dest, has_input):
        ...
        try:
            ...
            # 2. 构建pyinstaller命令，传递has_input参数和autosave_path
            pyinstaller_cmd = self.build_pyinstaller_command(app_name, icon_dest, has_input, autosave_path)
    ...
    def build_pyinstaller_command(self, app_name, icon_dest, has_input, autosave_path):
